...
import {liveOpening,referenceOpenness,type Racer} from './timeline';
...
export function RacingLeaderboard(props:RacingLeaderboardProps){
 ...
 useEffect(()=>{
  ...
  function frame(now:number){
   ...
   const liveClock=now/1000;const openingAge=opening?(now-opening.started)/1000:2;
   ...
   planes.forEach((plane,i)=>{
    const motionOpen=opening&&!p.replay&&!p.reducedMotion&&opening.index===i?liveOpening(openingAge):opens[i];const e=Math.max(0,Math.min(1.065,motionOpen));const u=plane.material.uniforms;u.uOpen.value=e;
    ...
    const anchor=20+120*Math.min(1,e);plane.position.x=300+(anchor-300)*(1-pulseX);
