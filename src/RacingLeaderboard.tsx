 ...
 useEffect(()=>{
  ...
  function frame(now:number){
   ...
   planes.forEach((plane,i)=>{
    ...
    const wallX=plane.position.x+(anchor-300)*pulseX;const row=rows.current[i];if(row){row.style.setProperty('--open',String(e));row.style.setProperty('--anchor',`${anchor}px`);row.style.setProperty('--pulse-x',String(pulseX));row.style.setProperty('--pulse-y',String(pulseY));row.style.setProperty('--text-shift-x',`${(wallX-anchor)/pulseX}px`);row.style.setProperty('--text-shift-y','0px');row.style.setProperty('--text-scale-x',String(1/pulseX));row.style.setProperty('--text-scale-y',String(1/pulseY));row.style.width=`${90+e*475}px`;}
